...
const QCBatchesPage = () => {
  ...
  // Fetch survey and batches (V2 - Optimized with pagination)
  const fetchData = async (page = 1) => {
    try {
      ...
      // Fetch batches using V2 API with pagination
      const batchesResponse = await qcBatchAPI.getBatchesBySurveyV2(surveyId, {
        page: page,
        limit: 20
      });
      ...
      if (batchesResponse.success) {
        ...
        if (batchesResponse.data.pagination) {
          setPagination(batchesResponse.data.pagination);
        }
      }
    } catch (error) {
  ...
  const handleViewBatch = async (batchId) => {
    try {
      const response = await qcBatchAPI.getBatchByIdV2(batchId, {
        responsePage: 1,
        responseLimit: 50
      });
      if (response.success) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading QC batches...</p>
        </div>
      </div>
    );
  }
              ...
              <div className="flex items-center space-x-4 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-gray-600">
                    {pagination.totalBatches || batches.length} batch{(pagination.totalBatches || batches.length) !== 1 ? 'es' : ''}
          ...
          {pagination.totalPages > 1 && (
            <div className="mt-6 flex items-center justify-between bg-white px-4 py-3 rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="text-sm text-gray-600">
                Showing {((pagination.currentPage - 1) * pagination.limit) + 1} to {Math.min(pagination.currentPage * pagination.limit, pagination.totalBatches)} of {pagination.totalBatches} batches
              </div>
            </div>
          )}
        </div>
