/**
 * Update CATI Interviewers from Excel File and Assign ACs
 * 
 * This script:
 * 1. Reads Excel file with phone numbers
 * 2. Finds quality agents by email/QC ID
 * 3. Updates existing CATI interviewers or creates new ones
 * 4. Updates phone numbers and passwords
...
const EXCEL_FILE = '/var/www/reports/App QC id (1).xlsx';
...
const REPORT_FILE = path.join(REPORT_DIR, `cati-update-from-excel-${Date.now()}.json`);
const REVERSAL_SCRIPT = path.join(REPORT_DIR, `reverse-cati-update-from-excel-${Date.now()}.js`);
...
/**
 * Extract phone number from email if it's phone-based
 */
...
function readExcelFile() {
  try {
    const pythonScriptPath = path.join(__dirname, '../../reports/temp_read_excel.py');
...
for _, row in df.iterrows():
    ...
    # Extract phone from email if it's phone-based
    if '@' in email:
        phone_part = email.split('@')[0]
        if phone_part.isdigit() and len(phone_part) == 10:
            phone_from_email = phone_part
    
    users.append({
        'name': str(row['Name']),
        'email': email,
        'qc_id': int(qc_id) if qc_id else None,
        'phone_from_email': phone_from_email
    })
...
async function updateOrCreateCATIInterviewer(userData, qualityAgent, acCode, acName, companyAdmin, referenceUser) {
  ...
  let actualPhone = userData.phone_from_email;
  if (!actualPhone && qualityAgent.phone) {
  ...
  if (qualityAgent.phone === actualPhone) {
    ...
    changes.qaPhoneModified = true;
    changes.qaPhoneModifiedTo = modifiedPhone;
  }
    ...
    for (let i = 0; i < excelUsers.length; i++) {
      ...
      try {
        ...
        const result = await updateOrCreateCATIInterviewer(
          { ...userData, phone_from_email: actualPhone },
          qualityAgent,
          acCode,
          acName,
          companyAdmin,
...
function generateReversalScript(report, scriptPath) {
  ...
  return `/**
 * Reversal Script: Revert CATI Interviewer Updates from Excel
 * 
