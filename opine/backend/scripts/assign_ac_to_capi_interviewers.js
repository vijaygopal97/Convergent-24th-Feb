...
/**
 * Assign ACs to CAPI Interviewers Script
 * 
 * This script assigns ACs to multiple CAPI interviewers by member ID
 * 
 * Usage:
 *   node assign_ac_to_capi_interviewers.js
 */
 ...
 */
async function assignACToCAPIInterviewer(surveyId, memberId, acCode, acMap, assignedByUserId) {
  // Find AC name
  ...
  if (!assignment) {
    ...
    console.log(`✅ Created new assignment for interviewer ${memberId}`);
  }
  
  // Check if AC is already assigned
  if (assignment.assignedACs.includes(acName)) {
    console.log(`⚠️  AC "${acName}" is already assigned to interviewer ${memberId}`);
    return {
      success: true,
  ...
  // Save survey
  await survey.save();
  console.log(`✅ AC "${acName}" assigned successfully to ${memberId}`);
...
async function main() {
  try {
    ...
    console.log('ASSIGN ACs TO CAPI INTERVIEWERS');
    console.log('='.repeat(70));
    ...
    for (let i = 0; i < MEMBER_IDS.length; i++) {
      ...
      console.log(`\n📊 Processing interviewer ${i + 1}/${MEMBER_IDS.length}: Member ID ${memberId}`);
      console.log(`   Assigning AC: ${acCode}`);
      
      try {
        const result = await assignACToCAPIInterviewer(surveyId, memberId, acCode, acMap, assignedByUserId);
        results.push(result);
      } catch (error) {
...
module.exports = { main, assignACToCAPIInterviewer, loadACJson, formatACCodeForLookup, findACName };
